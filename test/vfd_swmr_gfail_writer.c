...
state_init(state_t *s, socket_state_t *sock, int argc, char **argv)
{
    ...
    while ((ch = getopt(argc, argv, "SGNn:T:t:m:B:s:di:q")) != -1) {
        switch (ch) {
            case 'S':
 ...
 */
static bool
sock_wr_send_receive(state_t *s, socket_state_t *sock)
{
 ...
 */
static bool
sock_rd_receive(socket_state_t *sock)
{
 ...
 */
static bool
sock_rd_send(socket_state_t *sock)
{
 ...
 */
static bool
sock_send_error(socket_state_t *sock)
{
    sock->notify = -1;
...
main(int argc, char **argv)
{
    ...
    if (writer) {
        ...
        if (s->use_communication) {
            if (false == sock_rd_receive(sock)) {
                TEST_ERROR;
            }

            dbgf(2, "reader receives the message.\n");
            if (sock_rd_send(sock) == false) {
                TEST_ERROR;
            }
