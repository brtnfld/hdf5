...
**Date:** 2026-06-30  
**Last updated:** 2026-06-30 (applied fixes for items 2, 3, 5, 6, 7, 8; all readability items; H5CL issues marked won't-fix — replacing with TOML reader)  
...
| Sev | Status | Finding | Location |
| --- | --- | --- | --- |
| 🟠 High | ✅ | `H5MV` forks `H5MF` but **dropped the rollback** present in the original: on `H5FS_sect_add` failure it does not restore `node->sect_info.addr/size` (original restores them at `H5MF.c:773`) → inconsistent node / leaked or corrupted free-space accounting | `src/H5MV.c:239-241` |
...
| Sev | Status | Finding | Location |
| --- | --- | --- | --- |
| 🟠 High | ◐ | 8× over-allocation + `unsigned int` overflow: allocates `num * 64` / `num * 32` where `sizeof` gives `* 8` / `* 4` | `utils/vfd_swmr/aux_process.c:1419-1421` |
...
| 🟠 High | ✅ | **Stack overflow from argv:** `sprintf(updater_name, "%s.%d", hand->updater_path, i)` into 1024-byte buffer (recovery_tool.c already uses bounded `snprintf`) | `utils/vfd_swmr/aux_process.c:1502` | `snprintf` + check return |
| 🟠 High | ◐ | `strdup(getenv("PATH"))` then `strchr` with no NULL guard if `PATH` unset | `utils/vfd_swmr/recovery_tool.c:1407` | guard `getenv` result |
| 🟡 Med | ◐ | Release-stripped bounds checks: peer-controlled `index_length`/`num_entries`/page offsets validated only via `assert()` (compiled out under `NDEBUG`) on a shadow-file read path | `src/H5FDvfd_swmr.c:1018-1024`, `:1459`, `:1571-1591` | convert load-bearing asserts to runtime `HGOTO_ERROR` |
...
| 🟡 Med | ◐ | Updater VFD handle + temp file leaked when `H5F__generate_updater_file` errors after `H5FD_open` | `src/H5Fvfd_swmr.c:2307-2439` | `if (ud_file) H5FD_close(ud_file)` in `done:` |
...
| 🔵 Low | ◐ | Log `fclose` not followed by NULLing pointer → double-close on retry | `src/H5Fvfd_swmr.c:472-475` | null pointer + clear flag |
| 🔵 Low | ◐ | `sprintf` into fixed log buffers (safe today, brittle) | `src/H5Fvfd_swmr.c:1029`, `:2081` | `snprintf` |
...
- ◐ Edge cases to add: index realloc-failure leaves `mdf_idx_len`/`old_mdf_idx_len`
  inconsistent → latent over-read (`src/H5Fvfd_swmr.c:1242-1248`); `H5MV__sect_split` has no
  `frag_size < sect->size` guard → `hsize_t` underflow (`src/H5MVsection.c:361`); `do_sleep`
  divide-by-zero on `-p 0` (`utils/vfd_swmr/aux_process.c:443`, `atoi` unvalidated).
...
| 4 | 🔴 | **Recovery/aux-tool parser hardening** — validate all on-disk lengths/counts/offsets + NULL-check mallocs. *(Tenet 5, ◐)* | **Open** |
...
| 9 | 🟡 | **Remove dead 1,688-line macro block / duplicate `#define`** (breaks `-Werror`). *(Tenets 1-2, ✅/◐)* | **Open** |
