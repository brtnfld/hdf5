...
static inline int
copystring(char *dst, int dstsz, const char *src)
{
 ...
 */
static int
SETERROR(ebuf_t ebuf, int lineno, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    char *p = ebuf.ptr;
...
static void
scan_copystr(scanner_t *sp, char *dst, int dstsz)
{
    assert(dstsz > 0);
...
static int
tab_find(toml_datum_t *tab, span_t key)
{
...
static int
tab_add(toml_datum_t *tab, span_t newkey, toml_datum_t newvalue, const char **reason)
{
    assert(tab->type == TOML_TABLE);
...
// On error, *reason will point to an error message.
static toml_datum_t *
arr_emplace(toml_datum_t *arr, const char **reason)
{
...
static int
datum_copy(toml_datum_t *dst, toml_datum_t src, pool_t *pool, const char **reason)
{
...
static inline bool
is_array_of_tables(toml_datum_t datum)
{
...
static int
datum_merge(toml_datum_t *dst, toml_datum_t src, pool_t *pool, const char **reason)
{
...
static bool
datum_equiv(toml_datum_t a, toml_datum_t b)
{
    if (a.type != b.type) {
        return false;
    }
    ...
    switch (a.type) {
        case TOML_STRING:
            return a.u.str.len == b.u.str.len && 0 == memcmp(a.u.str.ptr, b.u.str.ptr, a.u.str.len);
        case TOML_INT64:
        ...
        case TOML_DATE:
            return a.u.ts.year == b.u.ts.year && a.u.ts.month == b.u.ts.month && a.u.ts.day == b.u.ts.day;
        case TOML_TIME:
            return a.u.ts.hour == b.u.ts.hour && a.u.ts.minute == b.u.ts.minute &&
                   a.u.ts.second == b.u.ts.second && a.u.ts.usec == b.u.ts.usec;
        case TOML_DATETIME:
            return a.u.ts.year == b.u.ts.year && a.u.ts.month == b.u.ts.month && a.u.ts.day == b.u.ts.day &&
                   a.u.ts.hour == b.u.ts.hour && a.u.ts.minute == b.u.ts.minute &&
                   a.u.ts.second == b.u.ts.second && a.u.ts.usec == b.u.ts.usec;
 ...
 */
toml_result_t
toml_merge(const toml_result_t *r1, const toml_result_t *r2)
{
    const char   *reason = "";
    toml_result_t ret    = {0};
    pool_t       *pool   = 0;
...
bool
toml_equiv(const toml_result_t *r1, const toml_result_t *r2)
{
    if (!(r1->ok && r2->ok)) {
        return false;
    }
 ...
 */
toml_datum_t
toml_get(toml_datum_t datum, const char *key)
{
    if (datum.type == TOML_TABLE) {
 ...
 */
toml_datum_t
toml_seek(toml_datum_t table, const char *multipart_key)
{
 ...
 */
void
toml_set_option(toml_option_t opt)
{
    toml_option = opt;
}
 ...
 */
void
toml_free(toml_result_t result)
{
 ...
 */
toml_result_t
toml_parse_file_ex(const char *fname)
{
    toml_result_t result = {0};
    FILE         *fp     = fopen(fname, "r");
    if (!fp) {
        snprintf(result.errmsg, sizeof(result.errmsg), "fopen %s: %s", fname, strerror(errno));
        return result;
    }
 ...
 */
toml_result_t
toml_parse(const char *src, int len)
{
    toml_result_t result = {0};
    parser_t      parser = {0};
    parser_t     *pp     = &parser;
    ...
    if (toml_option.check_utf8) {
        ...
        for (int i = 0; i < len;) {
            ...
            if (0xD800 <= ch && ch <= 0xDFFF) {
                ...
                snprintf(result.errmsg, sizeof(result.errmsg), "invalid UTF8 char \\u%04x on line %d", ch,
                         line);
                goto bail;
            }
            line += (ch == '\n' ? 1 : 0);
...
// datum.
static int
token_to_string(parser_t *pp, token_t tok, toml_datum_t *ret)
{
...
static int
token_to_timestamp(parser_t *pp, token_t tok, toml_datum_t *ret)
{
    (void)pp;
...
static int
token_to_int64(parser_t *pp, token_t tok, toml_datum_t *ret)
{
    (void)pp;
...
static int
token_to_fp64(parser_t *pp, token_t tok, toml_datum_t *ret)
{
    (void)pp;
...
static int
token_to_boolean(parser_t *pp, token_t tok, toml_datum_t *ret)
{
    (void)pp;
...
static int
parse_key(parser_t *pp, token_t tok, keypart_t *ret_keypart)
{
    ret_keypart->nspan = 0;
...
static toml_datum_t *
descend_keypart(parser_t *pp, int lineno, toml_datum_t *toptab, keypart_t *keypart, bool stdtabexpr)
{
...
static void
set_flag_recursive(toml_datum_t *datum, uint32_t flag)
{
    datum->flag |= flag;
    switch (datum->type) {
        case TOML_ARRAY:
...
// Parse a value.
static int
parse_val(parser_t *pp, token_t tok, toml_datum_t *ret)
{
...
// like [a.b.c.d].
static int
parse_std_table_expr(parser_t *pp, token_t tok)
{
    // std-table = [ key ]
    // Eat the [
...
// like [[a.b.c.d]].
static int
parse_array_table_expr(parser_t *pp, token_t tok)
{
    // array-table = [[ key ]]
...
static int
parse_keyvalue_expr(parser_t *pp, token_t tok)
{
    // Obtain the key
    int       keylineno = tok.lineno;
    keypart_t keypart;
    ...
    for (int i = 0; i < keypart.nspan - 1; i++) {
        ...
        if (value->type == TOML_ARRAY) {
            return SETERROR(pp->ebuf, keylineno, "encountered previously declared array '%s'",
                            keypart.span[i].ptr);
        }
...
static int
parse_norm(parser_t *pp, token_t tok, span_t *ret_span)
{
    ...
    while (*p) {
        ...
        switch (p[1]) {
            ...
            case '\n':
                ...
                continue;
            default:
                return SETERROR(pp->ebuf, tok.lineno, "internal: unknown escape char \\%c", p[1]);
        }
    }
    *dst          = 0;
...
static inline bool
scan_match(scanner_t *sp, int ch)
{
    const char *p = sp->cur;
    // exact match? done.
    if (p < sp->endp && *p == ch) {
...
static bool
scan_matchany(scanner_t *sp, const char *accept)
{
    for (; *accept; accept++) {
...
static inline bool
scan_nmatch(scanner_t *sp, int ch, int n)
{
    assert(ch != '\n'); // not handled
    if (sp->cur + n > sp->endp) {
        return false;
...
mktoken(scanner_t *sp, toktyp_t typ)
{
    token_t tok = {0};
    tok.toktyp  = typ;
    tok.str.ptr = sp->cur;
...
static int
scan_multiline_string(scanner_t *sp, token_t *tok)
{
    assert(S_MATCH3('"'));
...
static int
scan_string(scanner_t *sp, token_t *tok)
{
    assert(S_MATCH('"'));
    if (S_MATCH3('"')) {
...
static int
scan_multiline_litstring(scanner_t *sp, token_t *tok)
{
    assert(S_MATCH3('\''));
    S_GET(), S_GET(), S_GET(); // skip opening '''
...
static int
scan_litstring(scanner_t *sp, token_t *tok)
{
    assert(S_MATCH('\''));
    if (S_MATCH3('\'')) {
...
static bool
is_valid_time(int hour, int minute, int sec, int usec)
{
    if (!(0 <= hour && hour <= 23)) {
        return false;
    }
...
static int
read_date(const char *p, int *year, int *month, int *day)
{
    const char *pp = p;
    int         n;
    n = read_int(p, year);
...
static int
read_tzone(const char *p, char *tzsign, int *tzhour, int *tzminute)
{
    const char *pp = p;
...
static int
scan_time(scanner_t *sp, token_t *tok)
{
...
static int
scan_timestamp(scanner_t *sp, token_t *tok)
{
...
static int
process_numstr(char *buffer, int base, const char **reason)
{
    // squeeze out _
...
static int
scan_float(scanner_t *sp, token_t *tok)
{
...
static int
scan_number(scanner_t *sp, token_t *tok)
{
    const char *reason;
    char        buffer[50]; // need to accomodate "9_007_199_254_740_991.0"
    scan_copystr(sp, buffer, sizeof(buffer));
...
static int
scan_bool(scanner_t *sp, token_t *tok)
{
    char buffer[10];
    scan_copystr(sp, buffer, sizeof(buffer));
...
static inline bool
test_time(const char *p, const char *endp)
{
...
static inline bool
test_date(const char *p, const char *endp)
{
...
static inline bool
test_bool(const char *p, const char *endp)
{
...
static bool
test_number(const char *p, const char *endp)
{
...
static int
scan_nonstring_literal(scanner_t *sp, token_t *tok)
{
    int lineno = sp->lineno;
...
// Scan a literal
static int
scan_literal(scanner_t *sp, token_t *tok)
{
...
static scanner_state_t
scan_mark(scanner_t *sp)
{
...
static void
scan_restore(scanner_t *sp, scanner_state_t mark)
{
    assert(mark.sp == sp);
    sp->cur    = mark.cur;
...
static int
scan_next(scanner_t *sp, bool keymode, token_t *tok)
{
...
static int
check_overflow(scanner_t *sp, token_t *tok)
{
    switch (tok->toktyp) {
        case TOK_LBRACK:
            sp->bracket_level++;
...
static int
scan_key(scanner_t *sp, token_t *tok)
{
    if (sp->errmsg) {
        return -1;
    }
...
static int
scan_value(scanner_t *sp, token_t *tok)
{
    if (sp->errmsg) {
        return -1;
    }
 ...
 */
static int
utf8_to_ucs(const char *orig, int len, uint32_t *ret)
{
 ...
 */
static int
ucs_to_utf8(uint32_t code, char buf[4])
{
