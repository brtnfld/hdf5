...
//--------------------------------------------------------------------------
// Function:    DSetCreatPropList::appendFilter (string overload)
///\brief       Configures a filter with a key=value string and appends it to the pipeline
///\param       filter_id - IN: Filter identifier
///\param       flags     - IN: Filter flags (H5Z_FLAG_MANDATORY or H5Z_FLAG_OPTIONAL)
...
//--------------------------------------------------------------------------
// Function:    DSetCreatPropList::appendFilter (raw cd_values overload)
///\brief       Configures a filter with raw cd_values and appends it to the pipeline
///\param       filter_id - IN: Filter identifier
///\param       flags     - IN: Filter flags
///\param       cd_nelmts - IN: Number of elements in cd_values
///\param       cd_values - IN: Auxiliary data for the filter
///\exception   H5::PropListIException
//--------------------------------------------------------------------------
void
DSetCreatPropList::appendFilter(H5Z_filter_t filter_id, unsigned flags, size_t cd_nelmts,
...
//--------------------------------------------------------------------------
H5std_string
DSetCreatPropList::getFilterParams(unsigned filter_idx) const
{
    size_t params_len = 0;
    herr_t ret_value  = H5Pget_filter_params_by_idx(id, filter_idx, NULL, 0, &params_len);
    if (ret_value < 0)
        throw PropListIException("DSetCreatPropList::getFilterParams", "H5Pget_filter_params_by_idx failed");

    std::string buf(params_len + 1, '\0');
    ret_value = H5Pget_filter_params_by_idx(id, filter_idx, &buf[0], params_len + 1, &params_len);
    if (ret_value < 0)
        throw PropListIException("DSetCreatPropList::getFilterParams", "H5Pget_filter_params_by_idx failed");
